class V1::UserAssessmentsController < V1::BaseController
    ...
    def submit_answers
        ...
        if !a_id.present? || !answers.present?
        ...
        data.each do |d|
            q_id = d["question_id"]
            ques = AssessmentQuestion.where(question_id: q_id, assessment_id: a.id).first
            if ques.present?
                uar = UserAssessmentResponse.where(ser_id: @current_user.id,
                    assessment_id: a.id,
